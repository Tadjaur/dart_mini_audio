    ...
    for (iDecoder = 0; iDecoder < g_decoderCount; ++iDecoder) {
        if (!g_pDecodersAtEnd[iDecoder]) {
            ...
            if (framesRead < frameCount) {
                ...
                printf("frame %i\n", (int) ma_decoder_get_length_in_pcm_frames(&g_pDecoders[iDecoder]));
            }
        }
    }
...
int main(int argc, char** argv)
{
    ...
    */
    ...
    /* Now we start playback and wait for the audioma_device  thread to tell us to stop. */
    ...
    printf("current frame length: %i\n", ma_decoder_get_length_in_pcm_frames(&g_pDecoders[0]));
    printf("device stop: %i\n", ma_device_stop(&device));
    getchar();
    printf("device start: %i\n", ma_device_start(&device));
    getchar();
    printf("device start: %i\n", ma_device_start(&device));
    getchar();
    printf("device start: %i\n", ma_device_start(&device));
